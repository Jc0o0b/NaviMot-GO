...
class WeatherScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer2<WeatherProvider, RouteProvider>(
      builder: (context, weatherVM, routeVM, _) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text('Pogoda na trasie'),
            centerTitle: true,
          ),
          body: hasRoute && weatherVM.weatherPoints.isNotEmpty
              ...
              : Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        hasRoute ? 'Brak danych pogodowych' : 'Wyznacz trasę, aby zobaczyć pogodę',
  ...
  Widget _buildWeatherContent(BuildContext context, WeatherProvider weatherVM) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text('Prognoza na trasie', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _buildWeatherCard(weatherVM.weatherPoints[i]),
          ),
        ),
      ],
    );
  ...
  Widget _buildSummaryCard(BuildContext context, WeatherProvider weatherVM) {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
            Icon(_weatherIcon(condition), size: 64, color: _weatherColor(condition)),
            const SizedBox(height: 8),
            Text('${weatherVM.averageTemperature.toInt()}°C',
              style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold)),
            Text(_conditionName(condition), style: Theme.of(context).textTheme.bodyMedium),
            const SizedBox(height: 12),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildStat(Icons.air, '${weatherVM.maxWindSpeed.toInt()} km/h', 'Wiatr'),
                _buildStat(Icons.water_drop, '${weatherVM.maxPrecipitationProbability.toInt()}%', 'Opady'),
                _buildStat(Icons.location_on, '${weatherVM.weatherPoints.length}', 'Punkty'),
              ],
            ),
                ...
                child: Row(
                  children: [
                    Icon(
                      ...
                      color: weatherVM.hasRiskyWeather ? Colors.red : Colors.yellow,
                      size: 18,
                    ),
                    const SizedBox(width: 8),
                    Expanded(child: Text(weatherVM.weatherAlert!, style: const TextStyle(fontSize: 12))),
                  ],
                ),
              ),
            ],
    ...
    return Card(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('${wp.temperature.toInt()}°', style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Icon(_weatherIcon(wp.condition), color: _weatherColor(wp.condition), size: 24),
            const SizedBox(height: 4),
            Text('${wp.windSpeed.toInt()} km/h', style: const TextStyle(fontSize: 10, color: Colors.grey)),
          ],
        ),
      ),
    );
  ...
  String _conditionName(WeatherCondition c) {
    switch (c) {
      ...
      case WeatherCondition.partlyCloudy: return 'Częściowe zachmurzenie';
